...
APP_VERSION = "1.101 servicio 2 como librería"
...
def _candidate_template_paths() -> list[Path]:
    ...
    for basename in DEFAULT_TEMPLATE_BASENAMES:
        candidates.append(data_dir / basename)
        candidates.append(exe_dir / basename)
        candidates.append(exe_dir / "resources" / basename)
        candidates.append(exe_dir / "templates" / basename)
        candidates.append(exe_dir / "config" / basename)

    if hasattr(sys, "_MEIPASS"):
        base_pack = Path(sys._MEIPASS)
        for basename in DEFAULT_TEMPLATE_BASENAMES:
            candidates.append(base_pack / "resources" / basename)

    project_root = _project_root()
    for rel_path in DEV_TEMPLATE_PATHS:
...
class App(tk.Tk):
    ...
    def _build_header(self) -> None:
        ...
        tk.Label(
            header,
            text=(
                "1ª pasada: Limpieza •  2ª pasada: Asignación de productos "
                "(IA, librería interna servicio 2)"
            ),
            font=("Segoe UI", 10),
        ...
        tk.Label(
            ...
            font=("Segoe UI", 10),
        ).grid(row=0, column=2, sticky="ne", padx=(0, 8))

        logo_path = self._resolve_resource_first(
            DEV_LOGO_PATHS,
            packaged_subdir="resources",
        )
        if logo_path and logo_path.exists():
            try:
                self.logo_img = self._load_logo_small(
                    logo_path,
                    max_w=200,
                    max_h=48,
                )
                tk.Label(header, image=self.logo_img).grid(
                    row=0,
                    column=3,
                    rowspan=2,
                    sticky="ne",
    ...
    def _build_body(self) -> None:
        ...
        ttk.Label(
            ...
            text="Catálogo IA:",
            font=("Segoe UI", 10, "bold"),
        ).grid(row=0, column=3, sticky="w", padx=(16, 8))
        self.lbl_catalog_info = ttk.Label(
            card,
            text="Interno en librería servicio 2 (YAML)",
            font=("Segoe UI", 10),
        )
        ...
        ttk.Label(
            card,
            text="Template (REFCRU):",
            font=("Segoe UI", 10, "bold"),
        ).grid(row=1, column=0, sticky="w", padx=(4, 8), pady=(10, 0))
        self.entry_refcru_template = ttk.Entry(card)
        self.entry_refcru_template.grid(
            row=1,
            column=1,
            sticky="ew",
            padx=(0, 8),
        ...
        ttk.Label(
            card,
            text="(Exportado desde BC)",
            font=("Segoe UI", 9),
        ).grid(row=1, column=3, sticky="w", padx=(16, 8), pady=(10, 0))

        ttk.Label(
            card,
            text="Modelo IA:",
            font=("Segoe UI", 10, "bold"),
        ).grid(row=2, column=0, sticky="w", padx=(4, 8), pady=(10, 0))
        ...
        self.lbl_limits = ttk.Label(
            card,
            text=self._limits_text(),
            font=("Segoe UI", 9),
        )
        self.lbl_limits.grid(
            row=2,
            column=2,
            columnspan=3,
            sticky="w",
    ...
    def _resolve_resource_first(
        self,
        candidates: list[str],
        packaged_subdir: Optional[str] = None,
    ) -> Optional[Path]:
        base = _project_root()

        if hasattr(sys, "_MEIPASS") and packaged_subdir:
            for candidate in candidates:
                path = base / packaged_subdir / Path(candidate).name
                if path.exists():
                    return path
    ...
    def _auto_load_refcru_template(self) -> None:
        ...
        tried = "\n  - " + "\n  - ".join(str(path) for path in _candidate_template_paths())
        self._append(
    ...
    def _limits_text(self) -> str:
        ...
        return (
            f"Límites (orientativos): RPM={limits['RPM']}, "
    ...
    def _on_browse_refcru_template(self) -> None:
        ...
        if filename:
            self.refcru_template_path = Path(filename)
            self.entry_refcru_template.delete(0, "end")
    ...
    def _on_model_change(self, _evt=None) -> None:
        ...
        if selected in MODEL_PRESETS:
            ...
            self.model_limits = MODEL_PRESETS[selected].copy()
            self.lbl_limits.config(text=self._limits_text())
            self._apply_model_env()
            self._append(
                f"Modelo IA seleccionado: {selected}  →  {self._limits_text()}"
            )
    ...
    def _run_phase2_clicked(self) -> None:
        ...
        self._clear_log()
        self._append(
            f"Iniciando 2ª pasada (IA) con modelo {self.model_name} …"
        )
        self._append(
            "Catálogo gestionado internamente por la librería del servicio 2 (YAML interno)."
        )
        self._disable_actions()
        threading.Thread(
            target=self._run_phase2_thread,
            args=(cleaned,),
            daemon=True,
        ).start()
    ...
    def _run_clean_thread(self) -> None:
        ...
        try:
            ...
            ok = False
            self._append_async(f"ERROR: {exc}", tag="err")
        finally:
    ...
    def _format_progress_event(ev: Any, idx: int, total: int) -> str:
        ...
        try:
            ...
            if isinstance(ev, dict):
                old_code = ev.get("old_code") or ev.get("code") or ev.get("from") or "?"
                new_code = ev.get("new_code") or ev.get("mapped_code") or ev.get("to") or "?"
                conf = ev.get("confidence")
                if conf is None:
                    return f"{prefix}{old_code} → {new_code}"
                return f"{prefix}{old_code} → {new_code} ({float(conf):.2f})"
    ...
    def _run_phase2_thread(self, cleaned_bc3: Path) -> None:
        ...
        try:
            ...
            if not used:
                try:
                    run_phase2(**base_kwargs)
                except TypeError:
                    run_phase2(cleaned_bc3, None, out_phase2)
    ...
    def _ensure_input(self) -> bool:
        path_value = self.entry_input.get().strip()
        if not path_value:
            messagebox.showerror("Entrada", "Selecciona primero un fichero BC3.")
            return False
        self.input_path = Path(path_value)
